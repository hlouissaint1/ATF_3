    ...
    def __call__(self, obj):
        ...
        def pw_wrapper(self, env=None, session_user=None, **opts):
            ...
            try:
                ...
                raise AssertionError(f'Unable to log into Vault: {str(estr)}')
            ...
            self.common_server_hosts = f'{config.DOCROOT}/admin/{self.environment}_servers.xml'
            try:
                self.common_xml = etree.parse(self.common_server_hosts, PARSER)
            except Exception as estr:
                print(f'ERROR: reading server file {self.common_server_hosts}\n {str(estr)}')
                exit(1)
            self.server_hosts = f'{config.DOCROOT}/{self.session_user}/{self.environment}_servers.xml'
            try:
                self.xml = etree.parse(self.server_hosts, PARSER)
            except Exception as estr:
                print(f'ERROR: reading server file {self.server_hosts}\n{str(estr)}')
                exit(1)
            ...
            try:
                ...
                raise AssertionError(f'Malformed configuration file(s):\n\t{self.common_server_hosts} or \n\t{self.server_hosts}:\n\t {str(estr)}')
...
class Password:
    ...
    def get_credentials(self, **atts):
        ...
        for addr in self.vault_path:
                ...
                devs = self.xml.xpath(f'//{DEVICE(addr)}[@address="{addr}"]')
                if len(devs) == 0:
                        continue
                dev = devs[0]
...
#        if 'CERT_RO' in os.environ and os.environ['CERT_RO'] == 'True':
        ...
        try:
                ...
                raise AssertionError(f'ERROR: a certificate was found in vault but its path is missing or invalid\n\tnowhere to install certificate {path}}')
        except Exception as estr:
                raise AssertionError(f'ERROR: unknown error while attempting to read/write certificate {path}')
