	...
	abort_serviced = False
		...
		if abort_serviced == False and self.pid != 0:
    		...
    		if options.rest_session == True:
        		rest_var_overrides(options.__dict__)
...
class Make_CSV_Session:
	...
	if 'options' in evars and evars['options']['rest_session'] == True:
        	rest_var_overrides(evars['options'])


    def build_session_from_csv(self, csvfile, group=None, email=None, version=None, s_reset=False, run_session='yes'):
...
def rest_var_overrides(rvars):
	...
	if rvars['ip'] != None:
		os.environ['isensor_Name'] = rvars['ip']

def file_cleanup(user, keep, **opts):
	from os import stat
	udir = '%s/%s' % (DOCROOT, user)
	if user == 'tmp' or user == 'all':
		tmp_files = glob('/tmp/*.err')
		tmp_files.append(glob('/tmp/*.out'))
		tmp_files.append(glob('/tmp/*.var'))
		print ('%d tmp files to delete\n' % len(tmp_files))
		for tfile in tmp_files:
			ctime = stat(tfile).st_ctime
			if NOW() - ctime > keep: 
				os.unlink(tfile)
				print('%s deleted' % tfile)
		if user == 'tmp':
			return
	...
	if user == 'all':
		...
		users = userxml.findall('user')
		for usr in users:
			udir = '%s/%s' % (DOCROOT, usr)
			if os.path.exists(udir):
				err_files = glob('%s/*.test_errors.txt' % udir)
				print('%d files to delete\n' % len(err_files))
				for efile in err_files:
...
if __name__ == "__main__":
    from atf_locks import atf_lock
    global options
    ...
    optprsr.add_option('','--clean-all', action='store_true', dest='clean_all', default=False, help='remove all temporary and user error files')
    optprsr.add_option('','--clean-tmp', action='store_true', dest='clean_tmp', default=False, help='remove all temporary files')
    optprsr.add_option('','--clean-user', action='store_true', dest='clean_user', default=False, help='remove all user error files')
    optprsr.add_option('','--keep',action='store', dest='keep', default='2', help='when used with "clean" option, only remove files older than <KEEP> days')

    optprsr.add_option('','--title', action='store', dest='title', default=None, help='insert <title> into mail subject')
    optprsr.add_option('','--init-session', action='store_true', dest='init', default=False, help='Used by REST service only')
    ...
    keep = int(options.keep) * 86400
    if options.clean_user == True:
	file_cleanup(user, keep)
	exit(0)
    
    elif options.clean_tmp == True:
	file_cleanup('tmp', keep)
	exit(0)
    elif options.clean_all == True:
	file_cleanup('all', keep)
    	exit(0)
	...
	if options.testfile != None:
		...
		if not os.path.exists(csvfile):
			...
			logging.error(errstr)
			exit(1)
    		CSV = Make_CSV_Session(environment, user, options=options.__dict__)
		...
		if lock.startswith('ERROR'):
			...
			with open(test_errors, 'a') as ef:
				ef.write('%s\n' % lock)
			CSV.build_session_from_csv(options.testfile, options.group, options.email, options.version, options.reset, 'no')
			os.write(2, b'%s' % lock)
			exit(11)
		if testid == None:	
			testid = user + '-' + time.strftime('%Y%m%d_%H%M%S')
