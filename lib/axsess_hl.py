...
#MOUNT_POINT = lambda e: 'dm-%s' % e if e.lower() != 'production' else 'dm'
MOUNT_POINT = 'atf'
...
class vfetch():
    def __call__(self, obj):
        ...
        def pw_wrapper(self, env=None, session_user=None, **opts):
            ...
            self.common_server_hosts = '%s/admin/%s_servers.xml' % (DOCROOT, self.environment)
            try:
                ...
                exit(1)
            self.server_hosts = '%s/%s/%s_servers.xml' % (DOCROOT, self.session_user, self.environment)
	...
	matched = False
        for addr in self.vault_path:
                ...
                if len(devs) == 0:
                        continue
                dev = devs[0]
                if 'inactive' in dev.attrib.keys() and dev.attrib['inactive'] == 'yes':
                        continue
                dev_atts = dev.keys()
                att_keys = atts.keys()
                attchk = map(lambda x: True if att_keys[x] in dev_atts and atts[att_keys[x]] == dev.attrib[att_keys[x]] else False, range(0, len(att_keys)))
