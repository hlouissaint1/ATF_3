...
ASA_LOGPATH = '/var/www/cgi-bin/lib/logs'
DOCROOT = '/var/www/html/htdocs'
MODULE = 'scwxASAlib.py'
ASA_LOG = 'asa.log'

logging.basicConfig(
    format='%(asctime)s %(module)s:%(funcName)s:%(lineno)d - %(message)s',
    datefmt='%m/%d/%Y %I:%M:%S %p',
    filename='%s/%s' % (ASA_LOGPATH, ASA_LOG),
    filemode='w',
    level=logging.DEBUG)

ASALOG = logging.getLogger('asa_regression')
rhandler = logging.FileHandler('%s/%s' % (ASA_LOGPATH, ASA_LOG))
formatter = logging.Formatter('%(asctime)s %(module)s:%(funcName)s:%(lineno)d - %(message)s')
rhandler.setFormatter(formatter)
ASALOG.addHandler(rhandler)
ASALOG.setLevel(logging.DEBUG)
ASALOG.debug('Initialized logging')
...
class igniteCall(object):
    def __call__(self, pFunction):
        def ignite_api_call(self, obj_URL, headers, data=None, **opts):
            ASALOG.debug('requesting session token from FMC')
            # request a new token
            ...
            if resp.status_code != 200:
                ASALOG.error('failed to retrieve session token from Ignite @ %s: %d\n%s' % (
            ...
            if 'ERROR' in resp:
                ASALOG.error('ERROR refreshing access token: %s' % resp)
                return (resp)
            self.token = resp.content
            ASALOG.debug('Successfully retrieve session token from Cisco Ignite')
...
#                return ('ERROR return from API:\n%s\%s' % (str(estr), resp.text))
            response = resp
            ASALOG.debug('received response:\n%s' % response)
            return (response)
...
class Ignite(object):
    ...
    def __init__(self, **evars):
        ...
        ASALOG.debug('created Ignite instance for host @ %s' % self.ignite_IP)
    ...
    def igniteGET(self, url, headers=None, get_data=None, **opts):
        if 'parameters' in opts:
            ...
            parameters = {}
        ASALOG.info('sent GET request to Ignite: %s' % url)
    ...
    def ignitePUT(self, url, headers=None, put_data=None, **opts):
        ...
        ASALOG.info('sent PUT request to Ignite: %s with headers:\n%s, data:\n%s' % (url, self.headers, dumps(put_data)))
    ...
    def ignitePOST(self, url, headers=None, post_data=None, **opts):
        ...
        ASALOG.info('sent POST request to Ignite: %s\nPOST data:\n%s' % (url, dumps(post_data)))
        with open('asapost.txt', 'w') as f:
    ...
    @keyword()
    def Import_CTU_Ruleset_On_Ignite(self, ctu_ruleset_file=None, policy_type='ari-balanced', parent_policy='ATFlab_ASA5545_Performance_with_CTU_bal'):
        assert ctu_ruleset_file != None, 'ERROR: the CTU ruleset file name was not specified'
        ASALOG.debug('installing CTU ruleset to FMC %s: policy=%s, ctu_ruleset_file=%s' % (
        ...
        if "ERROR" in resp:
            return (resp)
        ASALOG.debug('CTU ruleset import successfully completed:\n%s' % resp)
        return (resp)
    
    
    @keyword()
    def Deploy_Policy_To_ASA_Device(self):
        url = '/policy/deploy'
        headers = {
            'accept': 'application/json',
            'devices': self.asa_Device,
            'blocking': 'false',
            'hostname': self.fmc_IP,
            'password': self.fmc_Password,
        ...
        ASALOG.debug('The headers values are: %s ' % headers)
        resp = self.igniteGET(url, headers)
        if "ERROR" in resp:
            return (resp)
        ASALOG.debug('CTU ruleset installation successfully completed on device %s:\n%s' % (self.fmc_IP, resp))
        return (resp)
            ...
            if time() - self.refresh_timer >= SESSION_TIMEOUT:
                ...
                if self.refresh_count >= MAX_REFRESH_COUNT:  # request a new token
                    ...
                    if resp.status_code != 204:
                        ASALOG.error('failed to retrieve session token from FMC @ %s: %d\n%s' % (
                    ...
                    self.refresh_timer = time()
            ASALOG.debug('successfully retrieve session token from FMC')
...
def non_fatal_error(rtext):
    ...
    if len(match) > 0:
        ASALOG.debug('FMC reported a non-fatal error:\n%s' % rtext)
        eresp = ''
        for m in match:
            eresp += m
        return ('WARNING: \n %s' % eresp)
...
class asaExpert(object):
    def __call__(self, pFunction):
        def asa_expert_call(self, **opts):
            warnings.simplefilter('ignore')
            paramiko.util.log_to_file('/var/www/cgi-bin/lib/paramiko.log')

            if not 'ftd_IP' in self.__dict__ or self.ftd_IP == None:
                return ('ERROR: the ASA device address was not supplied')
            asa = paramiko.SSHClient()
            asa.load_system_host_keys()
            asa.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            asa.connect(hostname=self.ftd_IP, username=self.ftd_User, password=self.ftd_Password, look_for_keys=False)
            try:
                interact = SSHClientInteraction(asa, timeout=1800, display=False)
                interact.expect(PROMPT)
                interact.send('expert\n')
                interact.expect(PROMPT)
            except Exception as estr:
                return ('ERROR: unable to establish a dialog with ASA @ %s:\n\t%s' % (self.ftd_IP, estr))
            try:
                rval = pFunction(self, interact, **opts)
            except Exception as estr:
        ...
        return (asa_expert_call)


class asaEnable(object):
    def __call__(self, pFunction):
        def asa_enable_call(self, command=None, **opts):
            warnings.simplefilter('ignore')
            paramiko.util.log_to_file('/home/atf/logs/paramiko.log')
            if not 'asa_IP' in self.__dict__ or self.asa_IP == None:
                return ('ERROR: the ASA secondary device address was not supplied')
            asa = paramiko.SSHClient()
            asa.load_system_host_keys()
            asa.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            try:
                asa.connect(hostname=self.asa_IP, username=self.asa_User, password=self.asa_Password,
                            look_for_keys=False)
            except Exception as estr:
                raise AssertionError, 'ERROR: unable to connect to %s using %s credentials:\n\t%s' % (
                    self.asa_IP, self.asa_User, str(estr))
            try:
                interact = SSHClientInteraction(asa, timeout=30, display=False)
                sleep(1)
                # interact.expect(PROMPT)
                ...
                asa.close()
                return ('ERROR: unable to establish a dialog in enable mode with ASA @ %s:\n\t%s' % (self.asa_IP, estr))
            try:
                ASALOG.debug('enabled command from %s:\n\t%s (%s)' % (pFunction.__name__, command, str(opts)))
                rval = pFunction(self, interact, command, **opts)
                ASALOG.debug('recvd: %s...' % rval[0:100])
                interact.send('exit\n')
                interact.expect(PROMPT)
            except Exception as estr:
                asa.close()
                return (
                        'ERROR: the enabled command sent to %s resulted in an exception:\ncalling function= %s\ncommand= %s\nexception=%s' % (
                    self.asa_IP, pFunction.__name__, command, estr))
            asa.close()
            return (rval)

        return (asa_enable_call)


class asaShell(object):
    def __call__(self, pFunction):
        def asa_shell_call(self, command=None, **opts):
            if not 'ftd_IP' in self.__dict__ or self.ftd_IP == None:
                return ('ERROR: the ASA device address was not supplied')
            asa = paramiko.SSHClient()
            asa.load_system_host_keys()
            asa.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            asa.connect(hostname=self.ftd_IP, username=self.ftd_User, password=self.ftd_Password)
            try:
                interact = SSHClientInteraction(asa, timeout=30, display=False)
                interact.expect(PROMPT)
            except Exception as estr:
        ...
        return (asa_shell_call)


class ASA():
    @keyword()
    def Get_Interface_List(self, **opts):
        resp = self.asa_enabled_command('show interface\n')
        if 'return_as_list' in opts and opts['return_as_list'] == True:
            ilist = re.findall('^(Interface.*)(\"\S+\")', resp, re.MULTILINE)
            rlist = map(lambda x: x[1].strip('"'), ilist)
            if 'exclude_mgmt_iface' in opts and opts['exclude_mgmt_iface'] == True:
                rlist.remove('Management')
                rlist.remove('inside')
            return (rlist)
    ...
    def Get_Interface_Stats(self, interfaces=None, **opts):
        ...
        interface_stats = {}
        rval = ''
        ASALOG.debug('interface list: %s' % str(interface_list))
        for interface in interface_list:
            attempts = 3
            while attempts > 0:
                ASALOG.debug('getting stats on interface %s' % interface)
                resp = self.asa_enabled_command('show interface %s' % interface)
                test_response = re.findall('(\d+)(?=\spackets\sinput)', resp, re.MULTILINE)
            ...
            if 'return_raw' in opts:
                return (resp)
            try:
                ASALOG.debug('parsing response %s' % resp)
                interface_stats[interface] = {
                    'pkts_in': re.findall('(\d+)(?=\spackets\sinput)', resp, re.MULTILINE)[0],
                    'pkts_out': re.findall('(\d+)(?=\spackets\soutput)', resp, re.MULTILINE)[0],
                    'pkts_dropped': re.findall('(\d+)(?=\spackets\sdropped)', resp, re.MULTILINE)[0],
                }
                ASALOG.debug('parsed response result= %s' % (str(interface_stats)))
                if interfaces != None:
                    for metric in interface_stats[interface]:
                        if 'return_%s' % metric in opts:
                            return (interface_stats[interface][metric])
                ASALOG.debug('adding "%s" to rval' % str(interface))
                rval += '\n%s' % interface
                rval += ''.join('\n\t%s=%s' % (stat.replace('_', ' '), interface_stats[interface][stat]) for stat in
                                interface_stats[interface])
                ASALOG.debug('rval defined as:\n%s' % rval)
            except Exception as estr:
    ...
    def Get_Traffic(self, interfaces=None, **opts):
        resp = self.asa_command('show traffic-statistics')
        if 'return_raw' in opts:
            return (resp)
        if 'Connection Error' in resp:
            return ('ERROR: unable to communicate with ASA device at %s' % self.ftd_IP)
        rval = ''
        if interfaces == None:
            ...
            interface_list = interfaces.split(',')
        for interface in interface_list:
            resp = self.asa_command('show traffic-statistics %s' % interface)
            ilines = resp.split('\n', 13)
    ...
    @keyword()
    def Get_Packet_Metrics(self, timestamp_str=None, end_time_str=None, wait_for_boundary=True, **opts):
        if not end_time_str:
            end_time = self.Get_ASA_Time(return_epoch=True)
        else:
            end_time = float(end_time_str)
        timestamp = float(timestamp_str)

        pstats = self.get_packet_stats(start_time=timestamp, end_time=end_time, wait_for_boundary=wait_for_boundary)
        ASALOG.debug('timestamp= %f, end_time= %f' % (timestamp, end_time))
        ASALOG.debug('performance counts: %s' % pstats)
        rval = 'recieved packets= %d\ndropped packets= %d\n time period: %d - %d\n' % (
            pstats['recv'], pstats['dropped'], float(timestamp), end_time)
        if pstats['recv'] > 0 and pstats['dropped'] == 0:
    ...
    @asaShell()
    def asa_command(self, dialog, command, **opts):
    ...
    @asaEnable()
    def asa_enabled_command(self, dialog, command, **opts):
        assert command != None, 'Missing command'
        dialog.send(command)
        dialog.expect(PROMPT)
        data = dialog.current_output_clean.strip('\n')
    ...
    def get_stats(self, dialog, columns, **opts):
        ...
        ASALOG.debug('start time= %s, end time= %s, columns= "%s"' % (start_time, end_time, columns))
        ...
        if wait_for_boundary == True:
            ...
            current_time = self.Get_ASA_Time(return_epoch=True)
            ASALOG.debug('time boundary= %s, current_time= %s' % (boundary, current_time))
            while current_time < boundary:
                current_time = self.Get_ASA_Time(return_epoch=True)
                sleep(1)
        mark = start_time - (start_time % 300)
        ASALOG.debug('time mark= %s' % mark)
        dialog.send('sudo su -')
        ...
        cmd = 'cat /var/sf/detection_engines/%s*/instance-*/now | grep -v "####" |cut -s -d "," -f %s |grep -v "#"' % (
            uuid, columns)
        ASALOG.debug('sending expert command to ASA at %s: \n%s\n' % (self.ftd_IP, cmd))
        dialog.send(cmd)
        dialog.expect(PROMPT)
        resp = dialog.current_output_clean.split('\n')
        ASALOG.debug('response was:\n%s...\n' % str(resp)[0:1000])
        return (resp)

    @asaExpert()
    def get_packet_stats(self, dialog, **opts):
        ...
        resp = self.get_stats(dialog, '1,61,62', **opts)
        pstats = {'recv': 0, 'dropped': 0, 'samples': [], 'null_samples': 0}
        for period in resp:
            valid = re.match('\d+,\d+,\d+', period)
            if not valid:
                continue
            times = period.split(',')
            if float(times[0]) < mark - 300:
                continue
            ASALOG.debug('using period record: %s' % period)
            if times[1] == '0' and times[2] == '0':
                pstats['null_samples'] += 1
            else:
                pstats['samples'].append(int(times[0]))
                pstats['recv'] += int(times[1])
                pstats['dropped'] += int(times[2])
        return (pstats)

    @keyword()
    def Get_ASA_CPU_Stats(self, start_time=None, end_time=None, usr_limitstr='30.0', sys_limitstr='10.0', **opts):
        if end_time == None or end_time == 'None':
            end_time = start_time
            start_time = str(float(end_time) - 300.0)
            ASALOG.debug('no end time supplied...using start= %s, end= %s' % (start_time, end_time))
        cstats = self.get_cpu_stats(start_time=start_time, end_time=end_time, **opts)
        ASALOG.debug('fetched CPU stats: \n%s' % str(cstats))
        ...
        for cpu in sorted(cstats[periods[0]]):
            rval += '%17s' % cpu
        rval += '\n%-9s' % '5m period'
        for cpu in sorted(cstats[periods[0]]):
            rval += '%9s%8s' % ('usr', 'sys')
        ...
        for period in periods:
            ...
            for cpu in sorted(cstats[period]):
                rval += '| %05.2f   %05.2f |' % (float(cstats[period][cpu]['usr']), float(cstats[period][cpu]['sys']))
                ...
                if float(cstats[period][cpu]['sys']) > sys_limit:
                    ...
                    result_val += '\nPeriod %s FAILED: a usage value for "sys" on one or more CPUs exceeded the specified limit of %4.2f' % (
                        tval, sys_limit)
        if result_val == '':
            result_val = '\nAll CPU values for "usr" were below the limit of %4.2f and all values for "sys" were below the limit of %4.2f' % (
                usr_limit, sys_limit)
            result_val += '\nUsage levels for all CPUs PASSED\n'
        return (rval + result_val)
    ...
    def get_cpu_stats(self, dialog, **opts):
        ...
        mark = start_time - (start_time % 300)
        ASALOG.debug('fetching cpu stats from %s to %s (mark= %s)' % (opts['start_time'], opts['end_time'], str(mark)))
        resp = self.get_stats(dialog, '1,27-45,61', **opts)
        rval = ''
        periods = {}
        ...
        for period in resp:
            ...
            if not valid:
                continue
            field = period.split(',')
            if float(field[0]) < mark - 300:  # only use the periods bracketed by the stat and end times
                continue
            if float(field[0]) > end_time + 300:
                continue
            if field[20] == '0' and include_idle_periods == False:  # filter out the periods with no traffic
                continue
            offset = 2
            periods[field[0]] = {}
            for cpu in sorted(cpus):
                periods[field[0]][cpu] = {'usr': field[offset], 'sys': field[1 + offset]}
                offset += 3
    ...
    def get_perfmon_stats(self, dialog, **opts):
        ...
        uuids = dialog.current_output_clean.strip('\n')
        uuid = re.findall('DE UUID\s+\:\s(\S+)', uuids, re.MULTILINE)[0]
        cmd = 'cat /var/sf/detection_engines/%s*/instance-*/now | sudo perfstats -q | pcregrep -o "^\s+Pkts (Recv|Drop).*(?<=\:)((\s+\d+){3})"' % uuid
        dialog.send(cmd)
        dialog.expect(PROMPT)
    ...
    @asaExpert()
    def get_ctu_local_rules_from_asa(self, dialog, **opts):
        ...
        fd, atf_tmp_file = mkstemp(prefix='asa', suffix='.rules')
        print
        'TMP:', atf_tmp_file
        asa = Connect(self.ftd_IP)
        print
        'CONNECTION OK'
    ...
    def fetch_rs_version_on_device(self, dialog, **opts):
        ...
        if not raw.find('No such file or directory') < 0:
            err = 'ERROR: no local rules file found on ASA device'
            ASALOG.error(err)
            return (err)
    ...
    @asaExpert()
    def get_asa_name(self, dialog, **opts):
    ...
    @asaExpert()
    def fetch_asa_time(self, dialog, **opts):
        cmd = 'date'
        dialog.send(cmd)
        dialog.expect(PROMPT)
        raw = dialog.current_output_clean
    ...
    @keyword()
    def Verify_ASA_Ruleset_Diffs(self, first, second):
        if int(first) == 0:
        ...
        rval = ''
        # varify that the added rules are on the ASA
        for added in diffs.xpath('signature[@diff="added"]'):
            ...
            if result == False:
                rval += 'ERROR: added ruleset could not be found on the ASA: signature - %s\n' % ''.join(
                    '%s:%s ' % (s, added.attrib[s]) for s in added.attrib)
            else:
                rval += 'PASSED: added ruleset was found on the ASA: signature = %s\n' % ''.join(
                    '%s:%s ' % (s, added.attrib[s]) for s in added.attrib)
        for removed in diffs.xpath('signature[@diff="removed"]'):
            ...
            if result == False:
                rval += 'ERROR: removed ruleset is still on ASA: signature - %s\n' % ''.join(
                    '%s:%s ' % (s, removed.attrib[s]) for s in removed.attrib)
            else:
                rval += 'PASSED: removed ruleset was not found on ASA: signature = %s\n' % ''.join(
                    '%s:%s ' % (s, removed.attrib[s]) for s in removed.attrib)
        ...
        for changed in diffs.xpath('signature[@diff="changed"]'):
            ...
            result, found = find_rule_on_device(swid=swid, expect=[True, True])
            if result == False:
                rval += 'ERROR: changed ruleset could not be found on the ASA: signature - %s\n' % ''.join(
                    '%s:%s ' % (s, changed.attrib[s]) for s in changed.attrib)
            else:
                rval += 'PASSED: changed ruleset was found on the ASA: signature = %s\n' % ''.join(
                    '%s:%s ' % (s, changed.attrib[s]) for s in changed.attrib)

        return (rval)

    @keyword()
    def Verify_ASA_CTU_Ruleset_Version(self, version=None, waittime='500'):
        ...
        if version != None:
            ...
            while time() < timeout:  # in case the FMC is slow in pushing down the ruleset...wait for it
                resp = self.fetch_rs_version_on_device()
                if 'ERROR' in resp:
                    return (resp)
                if int(resp) != int(version):
                    rval = 'FAILED: the ASA device @ %s is at version "%d" installed...not version "%d"' % (
                        self.ftd_IP, int(resp), int(version)
                    )
                else:
                    rval = 'PASSED: the ASA device @ %s is running ruleset version "%d"' % (self.ftd_IP, int(resp))
                    break
                sleep(int(waittime) / (1 + int(waittime) / 10))
        else:
            rval = '%d' % int(resp)
    ...
    @keyword()
    def Get_ASA_CTU_Ruleset_Version(self):
        return (self.Verify_ASA_CTU_Ruleset_Version())

    @keyword()
    def Get_ASA_CPU_Core_Utilization(self, limitstr='100.0'):
        limit = limitstr.strip('%')
        resp = self.asa_enabled_command('show cpu usage')
        if 'Connection Error' in resp:
            return ('ERROR: unable to communicate with ASA device at %s' % self.ftd_IP)
        return (self.check_asa_cpu_usage(resp, limit))

    def check_asa_cpu_usage(self, resp, limit_str):
        limit = float(limit_str)
        loads = re.findall('(\d+)(?=\%)', resp)
    ...
    @keyword()
    @keyword()
    def Get_ASA_Memory_Usage(self, limit='100.0'):
        raw = self.get_memory_data(limit=limit)
        return (raw)

    @keyword()
    def Get_ASA_Time(self, **opts):
        months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
        datestr = self.fetch_asa_time()
        ASALOG.info('retrieved timedate: %s from device' % datestr)
    ...
    @keyword()
    def Get_ASA_Model(self):
        resp = self.asa_command('show model')
        return (resp.rstrip())

    @keyword()
    def Get_ASA_Info(self):
        resp = self.asa_command('show version')
        return (resp)

    @keyword()
    def Get_SRU_Version(self, **opts):
        info = self.Get_ASA_Info()
        if 'return_raw' in opts:
            return (info)
    ...
    @keyword()
    def Present_Cause_Of_Failure(self, failstr, scrubstr):
        if failstr == scrubstr:
            return (failstr)
        return (failstr.replace(scrubstr, ''))
...
class FMC(ASA):
    ...
    def __init__(self, **evars):
        ...
        self.expect_buf = []
        self.interface_stats = {}
        ASALOG.debug('created FMC instance for host @ %s' % self.fmc_IP)
    ...
    def purge_ctu_rulesets(self, dialog, **opts):
        ...
        dialog.send(cmd)
        dialog.expect(PROMPT)
        ASALOG.debug('SENT: %s' % cmd)
        raw = dialog.current_output_clean
        ASALOG.debug('RESPONSE:\n%s\n' % raw)
        return (raw)
    ...
    def fmcGET(self, url, data=None, **opts):
        if 'parameters' in opts:
            ...
            parameters = {}
        ASALOG.info('sent GET request to FMC: %s' % url)
    ...
    def fmcPUT(self, url, put_data=None, **opts):
        self.request_body = put_data
        ASALOG.info('sent PUT request to FMC: %s with headers:\n%s, data:\n%s' % (url, self.headers, dumps(put_data)))
    ...
    def fmcPOST(self, url, post_data=None, **opts):
        self.request_body = post_data
        ASALOG.info('sent POST request to FMC: %s\nPOST data:\n%s' % (url, dumps(post_data)))
        with open('asapost.txt', 'w') as f:
            f.write(dumps(post_data))
    ...
    def Remove_Existing_Rulesets(self):
        ...
        if 'all done' in resp:
            ...
            rval += self.Deploy_Policy_To_ASA()
            return (rval)
    ...
    def Get_Interface_Device(self, deviceName=None, **opts):
        ...
        for device_name in self.devices:
            ...
            ASALOG.debug('fetching info on device: %s' % device_name)
    ...
    def Get_Policy_Assignments(self, **opts):
        ...
        rval = 'No policy found for ASA device @ %s' % self.ftd_IP
        if len(self.devices) == 0:
        ...
        for item in assignments['items']:
            ...
            for device in details['targets']:
                ...
                if device['name'] == self.asa.Name:
                    self.asa.Policy = policy
                    rval = policy.Name
        return (str(rval))
    ...
    def Set_Policy_Assignment(self, new_policy_name=None, policy_type='access'):
        ...
        J = JSONEncoder()
        assert new_policy_name != None, 'ERROR: the policy name to which the ASA is to be assigned was not specified'
        ...
        old_policy_assignment = self.asa.Policy.json
        try:
        ...
        ASALOG.debug('sending policy assignment request:\n%s' % send_json)
    ...
    @keyword()
    def Deploy_Policy_To_ASA(self, wait='1800.0'):
        sleep(120)
        return ('This keyword is deprecated with implementation of FAT utility')
        try:
            waittime = float(wait)
        except:
            ASALOG.error('Invalid wait time specified')
            waittime = 1800.0
        if not self.asa:
            self.Get_Device_Records()
        ...
        for item in deployable['items']:
            if item['device']['name'] != self.asa.Name:
                continue
        ...
        if is_deployable == False:
            ASALOG.info('Firepower device "%s" is not in a state to deploy policy "%s"...deploying anyway' % (
                self.asa.Name, policy_to_deploy))
        post_data = {
            "type": "DeploymentRequest",
            "version": version,
            "forceDeploy": True,
            "ignoreWarning": True,
            "deviceList": [self.asa.Id]
        }
        ...
        if 'metadata' in resp and 'task' in resp['metadata'] and 'id' in resp['metadata']['task']:
            ...
            timer = time() + waittime
            ASALOG.info('Deployment started, Task ID = %s. Waiting %d seconds to complete' % (taskID, int(waittime)))
            while time() < timer:
                job_status = self.Get_Job_Status(taskID)
                ASALOG.debug('job status: %s' % job_status['message'].replace(self.asa.Id, self.fp.Model))
                if job_status['message'].find('Deploying') >= 0 or job_status['message'].find(
                        'PARTIALLY_SUCCEEDED') >= 0:
                    ASALOG.debug(job_status['message'].replace(self.asa.Id, self.fp.Model))
                    sleep(5)
                    continue
                ...
                if job_status['message'].find('FAILED') >= 0:
                    return ('ERROR: %s' % job_status['message'].replace(self.asa.Id, self.fp.Model))
                timed_out = False
                break
            if timed_out == True:
                return ('ERROR: timed out after %d seconds waiting for policy to deploy' % int(timer))
            return (job_status['message'].replace(self.asa.Id, self.fp.Model))
    ...
    def Get_Device_Records(self, device_name=None, **opts):
        ...
        ASALOG.debug('fetching device rercords')
    ...
    def Get_Job_Status(self, jobid=None, **opts):
        ASALOG.debug('fetching job status for job %s' % jobid)
    ...
    def Upload_File_To_FMC(self, source=None, destination=None):
        ...
        ASALOG.info('copying %s on ATF to %s on FMC' % (source, destination))
    ...
    def Install_SRU_Rulesets(self, sru_ruleset_shell_script):
        ASALOG.debug('running SRU ruleset installation script "%s"' % sru_ruleset_shell_script)
        C = Connect(self.fmc_IP)
    ...
    def OLD_Install_CTU_Ruleset(self, ctu_ruleset_file=None, policy_type='ari-balanced', access_policy=None):
        ...
        ASALOG.debug('installing CTU ruleset: policy=%s, access=%s, ctu_ruleset_file=%s' % (
        ...
        ASALOG.debug('CTU ruleset installation Complete:\n%s' % resp)
        nfe = non_fatal_error(resp)
    ...
    def Download_CTU_Ruleset(self, version):
        ASALOG.debug('attempting to download CTU ruleset version %s' % version)
        V = VulnDB()
        vrt_ruleset, tmp_vrt_rulset = V.fetch_vrt_ruleset(version)
        if vrt_ruleset == None:
            return ('ERROR: downloading CTU rulesets:\n%s' % V.error)
        ASALOG.info('downloaded CTU Rulesets %s and stored in temporary file %s' % (vrt_ruleset, tmp_vrt_rulset))
    ...
    def Download_CTU_Ruleset_Diffs(self, first=None, second=None):
        ASALOG.debug('Arguments: %s, %s' % (str(first), str(second)))
...
class Connect:
    def __init__(self, address):
        ...
        try:
            ...
            ASALOG.debug('creating transport session for %s' % self.ip)
            T = Transport((self.ip, 22))
            ASALOG.debug('starting transport client for %s' % self.ip)
            T.start_client()
            key = T.get_remote_server_key()
        except Exception as estr:
            ASALOG.error('unable to insert isert host key for %s\n%s' % (self.ip, str(estr)))
        ...
        try:
            if keypath == None:
                ASALOG.debug('attempting connection to %s using password' % self.ip)
                self.cnx.connect(self.ip, username=user, password=pword, look_for_keys=False, allow_agent=False)
                ASALOG.debug("Connection established %s (%s) for user %s" % (device, self.ip, user))
                self.connection_established = True
            else:
                ASALOG.debug('attempting connection to %s using shared key @%s' % (self.ip, keypath))
                try:
    ...
    def cmd(self, command, **kwords):
        if self.connection_established == False:
            ASALOG.error('Connection error...cannot execute remote command')
            return ('Connection Error')
        # self.cnx = paramiko.SSHClient()

        if not kwords.has_key('logsuppress') or kwords['logsuppress'] == False:
            ASALOG.debug("Sent command '%s' to %s (%s)" % (command, self.device, self.ip))
        try:
            stdin, stdout, stderr = self.cnx.exec_command("%s" % command)
        except Exception as estr:
            ASALOG.debug('Error connecting to FMS: "%s"' % str(estr))
            self.error = ''
            self.reconnect()
            if self.error != '':
                ASALOG.error('Error connecting to FMS: %s' % self.error)
            stdin, stdout, stderr = self.cnx.exec_command("%s" % command)

        response = stdout.read()
        if response == '':
            response = stderr.read()
        ASALOG.debug("Rcvd response '%s' from device %s (%s)" % (response, self.device, self.ip))
        return (response)
    ...
    def reconnect(self):
        ...
        self.cnx.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        try:
            ASALOG.debug('reconnecting to host %s' % self.ip)
        ...
        ASALOG.debug('re-connection successful')

    def sudo(self, command, **flags):
        if self.connection_established == False:
            ASALOG.error('Connection error...cannot execute remote sudo')
            return ('Connection Error')
        # self.cnx = paramiko.SSHClient()
        flist = ''
        if len(flags) > 0:
        ...
        response = ''
        error = ''
        ASALOG.info('Sent sudo command %s to %s (%s)' % (command, self.device, self.ip))
        ...
        if error != '':
            response += '\nERROR:\n%s\n' % error
            ASALOG.error('ERROR in response to sudo command: %s' % "sudo -S %s 2>&1" % command)
        ASALOG.info("Rcvd response %s from device %s (%s)" % (response, self.device, self.ip))
    ...
    def pushfile(self, source, destination):
        ...
        if self.connection_established == False:
            ASALOG.error('Connection error...cannot upload file')
            return ('Connection Error')
        try:
            destination_path = os.path.basename(destination)
            ASALOG.info('uploading file %s from ATF to %s:%s' % (source, self.ip, destination))
            W = SCP.Write(self.transport, os.path.dirname(destination))
            W.send_file(source, destination_path)
            SCP.closing(W)
            C = Connect(self.ip)
            ASALOG.debug('moving %s/%s to %s' % (os.path.dirname(destination), os.path.basename(source), destination))
            resp = C.sudo('mv %s/%s %s' % (os.path.dirname(destination), os.path.basename(source), destination))
            ASALOG.debug('moved %s/%s to %s' % (os.path.dirname(destination), os.path.basename(source), destination))
            ASALOG.debug('comparing MD5 sums between source and destination files after upload')
            ...
            ASALOG.error('ERROR - failed to copy %s to %s (%s)' % (source, destination, str(estr)))
    ...
    def pullfile(self, destination, source):
        ...
        try:
            ...
            ASALOG.info('downloading file %s from %s:%s to ATF' % (source, self.ip, source))
