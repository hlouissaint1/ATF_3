...
def parse_testrun_XML(user, **opts):
    ...
    if 'id' in runnode.attrib:
	trid = runnode.attrib['id']
    	if tid != None and trid != tid:
		raise AssertionError, (HTML_NOT_FOUND, 'Test ID "%s" does not match test-in-progress or last completed test "%s"' % 
			(str(tid), str(trid))) 	
    logging.debug('testrun ID: %s, Report Request ID: %s' % (str(trid), str(tid)))
...
class ATF_Web_Service(BaseHTTPRequestHandler):
    ...
    def launch_testrun(self, jobj, request):
        logging.debug('testrun launch attempted: %s' % str(jobj))
        # logging.debug('calling function is %s line %d' % (inspect.stack()[1][3], inspect.stack()[1][2]))
